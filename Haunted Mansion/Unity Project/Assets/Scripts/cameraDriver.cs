...
public class cameraDriver : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		...
		else if (Input.GetAxis("Vertical") != 0 || Input.GetAxis("Horizontal") != 0 || Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D) || !Input.GetMouseButton(0) || !Input.GetMouseButton(1))
		{
			...
			float targetRotationAngleY = target.eulerAngles.x;
			float currentRotationAngleY = transform.eulerAngles.x;
			x = Mathf.LerpAngle(currentRotationAngle, targetRotationAngle, rotationDampening * Time.deltaTime);
			y = Mathf.LerpAngle(currentRotationAngleY, targetRotationAngleY, rotationDampening * Time.deltaTime);
		}
